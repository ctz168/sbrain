#!/usr/bin/env python3
"""
Telegram Bot - DeepSeek-R1-1.5B版本
...
class TelegramBot:
    ...
    def __init__(self, token: str, model_path: str, device: str = "cpu"):
        ...
        print("\n" + "=" * 60)
        print("DeepSeek-R1-1.5B Telegram Bot")
        print("=" * 60)
        
        from core.deepseek_bot import DeepSeekContinuousBot
        self.bot = DeepSeekContinuousBot(model_path=model_path, device=device)
        print("\n✓ Bot 就绪")
    
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        self.bot.reset()
        await update.message.reply_text(
            "🧠 DeepSeek-R1-1.5B 连续对话Bot\n\n"
            "特性：\n"
            "• 推理树注入\n"
            "• 上下文记忆\n"
            "• 重复检测\n"
...
def main():
    parser = argparse.ArgumentParser(description="DeepSeek-R1-1.5B Telegram Bot")
    parser.add_argument("--token", type=str, required=True, help="Telegram Bot Token")
    parser.add_argument("--device", type=str, default="cpu", help="设备")
    parser.add_argument("--model_path", type=str, default="./models/DeepSeek-R1-Distill-Qwen-1.5B", help="模型路径")
